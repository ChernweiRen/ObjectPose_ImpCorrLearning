...
def save_images():
    ...
    for obj_glb in obj_glbs:
        ...
        # =========================================================================
        # 【新增功能】强制将新导入物体的原点移动到几何中心 (Fix for positive XYZ)
        # =========================================================================
        # 这步操作必须在 find_root_object 之前做，确保后续的矩阵计算使用的是新原点
        bpy.ops.object.select_all(action='DESELECT')
        for obj in current_new_objs:
            # 只处理 Mesh 类型的物体 (Empty/Light/Camera 不需要改原点)
            if obj.type == 'MESH':
                obj.select_set(True)
                bpy.context.view_layer.objects.active = obj # 必须设为 Active 才能操作
                # 核心命令：将原点移到包围盒中心
                bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='BOUNDS')
                obj.select_set(False)
        
        # 必须更新一下场景，确保原点变化被应用，bbox 计算才会准确
        bpy.context.view_layer.update()
        # =========================================================================
        ...
        if obj_glb.endswith(fixed_obj_names + ".glb"):
            ...
            # # ========【诊断代码 START】========
            # print(f"\n[DEBUG] Checking Origin for: {fixed_obj_names}")
            
            # # 遍历该物体的所有网格部件
            # for part in fixed_obj_parts:
            #     if part.type == 'MESH':
            #         # bound_box 返回的是局部坐标下的 8 个角点
            #         # 我们只需要看 min (左下角) 和 max (右上角)
            #         bbox_min = Vector(part.bound_box[0]) # 第0个点通常是 Min
            #         bbox_max = Vector(part.bound_box[6]) # 第6个点通常是 Max
                    
            #         print(f"  > Mesh: {part.name}")
            #         print(f"    Local BBox Min: {bbox_min}")
            #         print(f"    Local BBox Max: {bbox_max}")
                    
            #         if bbox_min.x >= 0 and bbox_min.y >= 0 and bbox_min.z >= 0:
            #             print("    ⚠️ 警告: 原点在角落！(所有坐标均为正值)")
            #         else:
            #             print("    ✅ 正常: 原点在内部 (包含负值坐标)")
            # # ========【诊断代码 END】========
